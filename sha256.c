...
void convert_hash_to_string(BYTE *hash, BYTE *out_string, WORD out_string_len) {
	char tmp_string[SHA256_BLOCK_SIZE * 2] = {0};
	int i = 0;

	if(!hash || !out_string){
		printf("Parameter \"hash\" and \"out_string\" cannot be NULL.\n");
		return;
	}
...
void print_hash_value(BYTE *hash){
	...
	if(!hash){
		printf("Parameter \"hash\" cannot be NULL.\n");
		return;
	}
	for(i = 0; i < SHA256_BLOCK_SIZE; i++) {
		printf("%02x\t", hash[i]);
