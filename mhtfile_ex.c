...
int buildMHTFileFvByFixedLeaves(const char* in_data_file,
	...
	int out_subds_fd = -1;
	char out_subds_filename[MHT_FILENAME_MAXLEN] = {0};
	...
	memop_alloc_zero(&read_buf, ds_hdr.m_ds_block_size + 1);
	...
	for (i = 0; i < output_mhtfile_num; ++i) {
		// construct sub-dataset file name
		ptr_ch = strrchr(in_data_file, '.');
		...
		for (j = 0; j < leaf_num; ++j)  // create sub-ds files that contain leaf_num blocks
		{
			...
			write(out_subds_fd, read_buf, ds_hdr.m_ds_block_size);
		} // for j
		fo_close_mhtfile(out_subds_fd);
		process_all_elem_fv_new_ds_fmt(out_subds_filename,
									   out_mht_file_prefix,
									   &pQHdr,
									   &pQTail,
									   FALSE);
...
void process_all_elem_fv_new_ds_fmt(char* in_data_file,
	...
	fo_close_mhtfile(out_file_fd);
	//re-open output mht file for write/read
	out_file_fd = fo_open_mhtfile(old_mht_filename);
	if(out_file_fd < 0){
		debug_print(THIS_FUNC_NAME, "re-open out-mht-file failed");
		return;
	}
...
void combine_nodes_with_same_levels(PQNode *pQHeader, 
	...
	if(!(*pQHeader)) {debug_print(THIS_FUNC_NAME, "pQHeader is null"); return;}
	if(!(*pQ)) {debug_print(THIS_FUNC_NAME, "pQ is null"); return;}
	if((*pQHeader) == (*pQ)) {debug_print(THIS_FUNC_NAME, "queue is null"); return;}
